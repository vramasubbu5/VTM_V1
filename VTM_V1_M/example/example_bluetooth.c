...
* Description:
* ------------
*   This example demonstrates how to program AUDIO interface in OpenCPU.
*
...
*     Download image bin to module to run.
    ...
    1.active side
      ...
      (3)scan the bt devices around ,and device info will be offered in URCS
      (4)choose one bt device to connect,if module has connected it before ,directly goes to step 7
      (5)for the first time to connect,you need make a pair between module and BT device
      ...
      (9)last step is to fulfil BT read/write
            ...
            if (UART_PORT2 == port)
            {
                ...
                if(0 == Ql_memcmp(m_RxBuf_Uart1,"name=",5)) 
                {
                    ...
                    if( p1 && p2)
                    {
                        ...
                        ret = RIL_BT_SetName(bt_name,sizeof(bt_name));
                        if(RIL_AT_SUCCESS == ret)
                        {
...
static void BT_Callback(s32 event, s32 errCode, void* param1, void* param2)
{
    ...
    switch(event)
    {
        case MSG_BT_SCAN_IND :
            ...
            if(BT_ERROR_CODE_SCAN_FINISH == errCode)
            {
            ...
            if(BT_ERROR_CODE_SCAN_FOUND == errCode)
            {
         ...
         case MSG_BT_PAIR_IND :
              if(BT_ERROR_CODE_PAIR_PINCODE == errCode)
              {
              ...
              else if(BT_ERROR_CODE_PAIR_NOPINCODE == errCode)
              {               
                    //no need pincode
                    pstconBtdev = (ST_BT_BasicInfo*)param1;
         ...
         case MSG_BT_PAIR_CNF_IND :
            if(BT_ERROR_CODE_PAIR_SUCCESS == errCode)
            {
                pstconBtdev = (ST_BT_BasicInfo*)param1;
         ...
         case MSG_BT_SPP_CONN_IND :
              ...
              if(BT_ERROR_CODE_CONN_SUCCESS == errCode )
              {
         ...
         case MSG_BT_PAIR_REQ:
              ...
              if(BT_ERROR_CODE_PAIR_NOPINCODE == errCode)
              {
                    //no need pincode
                    pstconBtdev = (ST_BT_BasicInfo*)param1;
                    Ql_strncpy(pinCode,(char*)param2,BT_PIN_LEN);
       ...
       case  MSG_BT_DISCONN_IND :
             ...
             if(BT_ERROR_CODE_DISCONN_OK == errCode)
             {
