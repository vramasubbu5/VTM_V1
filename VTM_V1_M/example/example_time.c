...
void proc_main_task()
{
    ...
    if(ret <0 )
    {
        APP_DEBUG("<--failed!!, Set local time (%i.%i.%i %i:%i:%i timezone=%i) -->\n\r", ret, time.day, time.month, time.year, time.hour, time.minute, time.second,time.timezone);
    }
    APP_DEBUG("<-- Set local time result: %i(%i.%i.%i %i:%i:%i timezone=%i) -->\n\r", ret, time.day, time.month, time.year, time.hour, time.minute, time.second,time.timezone);
